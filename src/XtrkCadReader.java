...
/**
 * Utility program
 * Converts layout schemes produced by the Open Source program XtrkCAD
 * (freely available from <a href="http://www.xtrkcad.org">http://www.xtrkcad.org</a> )
...
public class XtrkCadReader {
            ...
            if (blockIdent > 1) {
                ...
                for (i = startBlock; i < blockIdent; i++) {
                    ...
                    if (setBlockSensors && blockName.user.length() > 1) {
                        sensorName = "\" occupancysensor=\"S" + blockName.user.substring(1);
                    }
    ...
    public static void main(String[] args) {
        ...
        if (optionHeight.present && optionHeight.doubleValue >= 1.0) {
            jmriMaxHeight = optionHeight.doubleValue;
        }
        if (optionWidth.present && optionWidth.doubleValue >= 1.0) {
            jmriMaxWidth = optionWidth.doubleValue;
        }
        ...
        if (optionTolerance.present) {
            tolerance = optionTolerance.doubleValue;
        }
        if (optionHiddenDash.present) {
            hiddenDash = true;
        }
        ...
        if (optionAnchors.present) {
            anchorIdent = optionAnchors.intValue;
        }
        if (optionTracks.present) {
            trackIdent = optionTracks.intValue;
        }
        if (optionTurnouts.present) {
            turnoutIdent = optionTurnouts.intValue;
        }
        if (optionXings.present) {
            xingIdent = optionXings.intValue;
        }
        if (optionBumpers.present) {
            bumperIdent = optionBumpers.intValue;
        }
        if (optionTurntables.present) {
            turntableIdent = optionTurntables.intValue;
        }
        if (optionBlocks.present && optionBlocks.intValue > 0) {
            startBlock = optionBlocks.intValue;
        }
        if (optionTBlocks.present) {
            enableBlockTurnouts = true;
        }
        if (optionGBlocks.present) {
            enableBlockGaps = true;
        }
        if (optionRBlocks.present) {
            maxRange = optionRBlocks.doubleValue;
        }
        if (optionXBlocks.present) {
            enableBlockXing = true;
        }
        if (optionNBlocks.present) {
            getBlockNames = true;
        }
        if (optionSBlocks.present) {
            setBlockSensors = true;	// Version 1.3
        }		// And now do the job!
        new XtrkCadReader();
...
// INTERNAL CLASSES
                ...
                if (enableBlockXing) {
                    ...
                    if (anchor != null && ((anchor.blockGap & gapMask) == 0)) {
                        if (anchor.getConnectedTrack(1) != null) {
                            ...
                            if (!name1.equals("")) {
                                name1 = " Connection1=\"" + name1 + "\"";
                            }
                            String name2 = anchor.getConnectedName(1);
                            if (!name2.equals("")) {
                                name2 = " Connection2=\"" + name2 + "\"";
                            }
                ...
                case CROSSING:
                    ...
                    if (anchor1 != null) {
                        if (Math.pow(anchor1.x - x0, 2) + Math.pow(anchor1.y - y0, 2) <= range2) {
                            if ((anchor1.blockGap & gapMask) != 0 || anchor1.ref[1] == 0) {
                                return true;
                            }
                    ...
                    for (int ind = firstAnchor; ind < lastAnchor; ind++) {
                        ...
                        if (anchor.ref[1] != caller) {
                            if (Math.pow(anchor.x - x0, 2) + Math.pow(anchor.y - y0, 2) <= range2) {
                                // Version 1.3 Start
                                // Corrected error in block boundaries detection
                                if ((anchor.blockGap & gapMask) != 0 || anchor.ref[1] == 0) {
                                    return false;
                                }
                                // Version 1.3 End
    ...
    public class XtrkCadAnchor {
        ...
        public XtrkCadAnchor(int callingItem, boolean otherRef) {
            ...
            a = line.nextDouble();
            if (line.hasNext() && ((line.nextInt() & 256) != 0)) {
                blockGap = 1;
            }
        }
        ...
        public String getConnectedName(int ind) {
            ...
            // Attention: ind is not checked. It must be in the range 0-1
            XtrkCadElement track;
            if ((track = getConnectedTrack(ind)) != null) {
        ...
        public String getIdent() {
            ...
            if (duplicate < 0 || !skip) {
                ...
                if (duplicateAnchor.type == 2) {
                    return "EB" + duplicateAnchor.jmriNumber;	// Bumper (actually a bumper can unlikely have duplicates!)
                }
                if (duplicateAnchor.type > 6) {
                    return "X" + duplicateAnchor.jmriNumber;	// Crossing
                }
        ...
        public void print() {
            ...
            if (!skip && !printed && type < 50) {
                ...
                if (!name1.equals("")) {
                    name1 = " connect1name=\"" + name1 + "\"";
                }
                String name2 = getConnectedName(1);
                if (!name2.equals("")) {
                    name2 = " connect2name=\"" + name2 + "\"";
                }
